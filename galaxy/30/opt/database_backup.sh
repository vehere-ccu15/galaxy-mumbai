...
generate_post_data()
{
  cat <<EOF
{
 "indices": "$TODAYSNAPSHOT,logvehere-monitor-*,.kibana,addressbook,imported-pcap,userroles,vnfsdpi,vnfs-alert-status,scheduler,acknowledgedalerts",
...
curl -XPUT "http://192.168.30.30:9200/_snapshot/my_backup/$SNAPSHOT?wait_for_completion=true" -H 'Content-Type: application/json' -d"$(generate_post_data)" > ${LOGFILE}
